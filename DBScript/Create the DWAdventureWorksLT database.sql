--*********************************************************--
-- This file will drop and create the DWAdventureWorksLT2012
-- database, with all its objects. 
--********************************************************************--

USE [master];
If Exists (Select Name from SysDatabases Where Name = 'DWAdventureWorksLT2012')
  Begin
   Alter database DWAdventureWorksLT2012 set single_user with rollback immediate;
   Drop database DWAdventureWorksLT2012;
  End
go
CREATE DATABASE DWAdventureWorksLT2012;
go
...
While @DateInProcess <= @EndDate
	...
	--Add a row into the date dimension table for this date
		Insert Into [DWAdventureWorksLT2012].[dbo].[DimDates] 
		( [CalendarDateKey]
		, [CalendarDateName]
		, [CalendarYearMonthID]
...
--	JOIN [DWAdventureWorksLT2012].[dbo].[DimCustomers] as T3
--		ON T2.[CustomerID] = T3.[CustomerID]
--	JOIN [DWAdventureWorksLT2012].[dbo].[DimProducts] as T4
--		ON T4.[ProductID] = T1.[ProductID]
--	JOIN [DWAdventureWorksLT2012].[dbo].[DimDates] as T5
--		ON Cast(T5.CalendarDate as Date) = Cast(T2.[OrderDate] as Date)
--	JOIN [DWAdventureWorksLT2012].[dbo].[DimDates] as T6
...
--  Begin Transaction; 
--  -- ETL Code  -------------------------------------------------------------------
--		INSERT INTO [DWAdventureWorksLT2012].[dbo].[DimProducts]
--		( [ProductID]
--		, [ProductName]
--		, [ProductColor]
--		, [ProductListPrice]
...
--			,[ProductCategoryID]
--			,[ProductCategoryName]
--		FROM  [DWAdventureWorksLT2012].[dbo].[vETLDimProductsData]
--  -- ETL Code  -------------------------------------------------------------------
--  Commit Transaction;
--  Set @RC = 100; -- Success
-- End Try
...
--  Begin Transaction; 
--  -- ETL Code  -------------------------------------------------------------------
--		INSERT INTO [DWAdventureWorksLT2012].[dbo].[FactSales]
--		( [SalesOrderID]
--		, [SalesOrderDetailID]
--		, [CustomerKey]
--		, [ProductKey]
...
--			, [UnitPrice]
--			, [UnitPriceDiscount]
--			FROM [DWAdventureWorksLT2012].[dbo].[vETLFactSalesData] 
